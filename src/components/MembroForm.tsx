...
export function MembroForm({
  ...
  valoresIniciais?: Partial<MembroFormValues>;
  modo: "criar" | "editar";
  membroId?: number;
}) {
  const router = useRouter();
  const [valores, setValores] = useState<MembroFormValues>({ ...VAZIO, ...valoresIniciais });
  const [erro, setErro] = useState<string | null>(null);
  const [salvando, setSalvando] = useState(false);

  function atualizar<K extends keyof MembroFormValues>(campo: K, valor: MembroFormValues[K]) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const url = modo === "criar" ? "/api/membros" : `/api/membros/${membroId}`;
  ...
  return (
    <form onSubmit={handleSubmit} className="card max-w-lg space-y-4">
      ...
      <div className="flex gap-3">
        ...
        <button type="button" className="btn-secondary" onClick={() => router.push("/membros")}>
          Cancelar
        </button>
      </div>
    </form>
